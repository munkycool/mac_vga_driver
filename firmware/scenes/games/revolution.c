...
static const char *TRANS_QUOTES[] = {
    "GAY LIBERATION\nBUT TRANSGENDER\nNOTHING!\n- SYLVIA RIVERA",
    "WE HAVE TO BE\nVISIBLE. WE ARE\nNOT ASHAMED OF\nWHO WE ARE.\n- SYLVIA RIVERA",
    "THE REVOLUTION\nHAS ALWAYS BEEN\nQUEER & TRANS!\n- STONEWALL '69",
    "YOU NEVER REALLY\nHAVE YOUR RIGHTS\nUNTIL YOU ALL\nHAVE THEM.\n- M. P. JOHNSON"
...
// Draw a transgender symbol with black shadow background and flag colors inner highlights
static void draw_trans_symbol(uint8_t *buf, int cx, int cy) {
    // 1. Black silhouette background to ensure readability over waving flag
    // Circle
    draw_circle(buf, cx, cy, 22, 0);
    
    // Bottom cross (female)
    draw_rect(buf, cx - 4, cy + 20, cx + 4, cy + 38, 0);
    draw_rect(buf, cx - 12, cy + 26, cx + 12, cy + 32, 0);
    
    // Top-right arrow (male)
    for (int i = 0; i < 14; i++) {
        draw_rect(buf, cx + 12 + i, cy - 12 - i, cx + 18 + i, cy - 6 - i, 0);
    }
    draw_rect(buf, cx + 22, cy - 30, cx + 32, cy - 20, 0);
    
    // Top-left arrow with cross (transgender)
    for (int i = 0; i < 14; i++) {
        draw_rect(buf, cx - 18 - i, cy - 12 - i, cx - 12 - i, cy - 6 - i, 0);
    }
    draw_rect(buf, cx - 32, cy - 30, cx - 22, cy - 20, 0);
    draw_rect(buf, cx - 24, cy - 16, cx - 16, cy - 24, 0); // crossbar
    
    // 2. Inner colored details (Cyan 6, Pink 5, White 7)
    // Ring: outer radius 18, inner radius 13
    for (int y = -18; y <= 18; y++) {
        for (int x = -18; x <= 18; x++) {
            int d2 = x*x + y*y;
            if (d2 >= 13*13 && d2 <= 18*18) {
                uint8_t col = 7;
                if (y < -11 || y > 11) col = 6;      // Cyan
                else if (y < -4 || y > 4) col = 5;   // Pink
                else col = 7;                        // White
                
                draw_pixel(buf, cx + x, cy + y, col);
            }
        }
    }
    
    // Color the bottom cross (female) - Pink stem, White crossbar
    draw_rect(buf, cx - 1, cy + 21, cx + 1, cy + 37, 5); // Pink vertical
    draw_rect(buf, cx - 9, cy + 28, cx + 9, cy + 30, 7); // White horizontal
    
    // Color the top-right arrow (male) - Cyan
    for (int i = 0; i < 12; i++) {
        draw_pixel(buf, cx + 14 + i, cy - 10 - i, 6);
        draw_pixel(buf, cx + 15 + i, cy - 9 - i, 6);
    }
    draw_rect(buf, cx + 24, cy - 28, cx + 29, cy - 26, 6);
    draw_rect(buf, cx + 27, cy - 28, cx + 29, cy - 23, 6);

    // Color the top-left arrow (trans) - Cyan/Pink
    for (int i = 0; i < 12; i++) {
        draw_pixel(buf, cx - 14 - i, cy - 10 - i, 6);
        draw_pixel(buf, cx - 15 - i, cy - 9 - i, 6);
    }
    draw_rect(buf, cx - 29, cy - 28, cx - 24, cy - 26, 6);
    draw_rect(buf, cx - 29, cy - 28, cx - 27, cy - 23, 6);
...
void play_revolution(uint8_t *buf, int frame_counter) {
    ...
    if (current_subpage == 0) {
        ...
        for (int y = 0; y < 240; y += 4) {
            for (int x = 0; x < 320; x += 4) {
                ...
                else if (stripe == 1 || stripe == 3) color = 5; // Pink/Magenta
                else color = 7;                                 // White
    ...
    int curr_y = 182;
    int scale = 1;
    uint8_t text_color = 7;
    if (current_subpage == 0) text_color = 3;      // Yellow
